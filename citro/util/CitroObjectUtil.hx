...
class CitroObjectUtil {
     ...
     * @param arrayC An array of 32-bit RGBA color of the vertex of the triangle.
    ...
    public static function drawTriangle(arrayX:Array<Float>, arrayY:Array<Float>, arrayC:Array<UInt32>, drawBottom:Bool = false):Bool {
        if (arrayX.length > 2 && arrayY.length > 2 && arrayC.length > 0) {
            ...
            untyped __cpp__('
                ...
                C2D_DrawTriangle(
                    (*arrayX)[0], (*arrayY)[0], (*arrayC)[0],
                    (*arrayX)[1], (*arrayY)[1], (*arrayC)[1],
                    (*arrayX)[2], (*arrayY)[2], (*arrayC)[2],
                    1
                )
            ');
            return true;
     ...
     * @param arrayC An array of the Color's 32-bit RGBA color of the vertex of this line.
    ...
    public static function drawRect(x:Float, y:Float, w:Float, h:Float, color:Array<UInt32>, drawBottom:Bool = false):Bool {
        untyped __cpp__('C2D_SceneBegin(drawBottom ? bottomScreen : topScreen)');
        return color.length > 1 ? {
            while (color.length != 4) {
                color.push(color[0]);
            }
            untyped __cpp__('C2D_DrawRectangle(x, y, 1, w, h, (*color)[0], (*color)[1], (*color)[2], (*color)[3])');
            true;
        } : false;
    }
